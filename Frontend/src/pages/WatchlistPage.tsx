    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-5">
        ...
        <div className="flex items-center gap-3">
          <div className="bg-white border border-slate-200 px-4 py-2 rounded-xl shadow-2xs text-xs font-semibold text-slate-600">
            Saved: <span className="text-blue-600 font-bold">{watchlistStocks.length} Stocks</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-2xs">
          <span className="text-xs text-slate-400 font-medium block mb-1">Total Saved Assets</span>
          <span className="text-2xl font-extrabold text-slate-900">{watchlistStocks.length}</span>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-2xs">
          <span className="text-xs text-slate-400 font-medium block mb-1">Avg 7D Projected Return</span>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-2xs">
          <span className="text-xs text-slate-400 font-medium block mb-1">Top Conviction Forecast</span>
      ...
      {watchlistStocks.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center shadow-xs">
          ...
          <h3 className="text-lg font-bold text-slate-800 mb-1">Your Watchlist is empty</h3>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-lg font-extrabold text-slate-900">{stock.symbol}</span>
                  ...
                  <div className="bg-purple-50/70 border border-purple-100 rounded-xl p-3 mb-4">
                    ...
                    <div className="flex items-baseline justify-between text-xs text-slate-600">
                      <span>Target: <strong className="text-slate-900">{forecast7D.projectedPrice}</strong></span>
                      <span>Confidence: <strong className="text-slate-900">{forecast7D.confidence}%</strong></span>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full mt-2 py-2 px-3 bg-slate-50 hover:bg-blue-50 text-slate-700 hover:text-blue-700 font-semibold text-xs rounded-xl border border-slate-200 hover:border-blue-200 transition-all flex items-center justify-center gap-1.5"
                >
                  View Full Analytics
