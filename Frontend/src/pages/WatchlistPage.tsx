...
import { Star, TrendingUp, ArrowUpRight, ArrowDownRight, Trash2, ArrowRight, Search, ArrowUpDown } from 'lucide-react';
...
export const WatchlistPage: React.FC<WatchlistPageProps> = ({
  ...
  const [sortBy, setSortBy] = useState<'symbol' | 'price' | 'return'>('return');
  ...
  const filteredStocks = watchlistStocks
    ...
    .sort((a, b) => {
      ...
      // sort by 7D return percentage
      const retA = parseFloat(a.timeframes['7D']?.percentageChange.replace('%', '').replace('+', '') || '0');
      const retB = parseFloat(b.timeframes['7D']?.percentageChange.replace('%', '').replace('+', '') || '0');
      return retB - retA;
    });

  // Calculate true dynamic average 7D projected return across all saved stocks
  const avg7DReturnNum =
    watchlistStocks.length > 0
      ? watchlistStocks.reduce((sum, s) => {
          const val = parseFloat(s.timeframes['7D']?.percentageChange.replace('%', '').replace('+', '') || '0');
          return sum + (isNaN(val) ? 0 : val);
        }, 0) / watchlistStocks.length
      : 0;

  const avg7DReturnFormatted = `${avg7DReturnNum >= 0 ? '+' : ''}${avg7DReturnNum.toFixed(1)}%`;

  // Dynamic Top Conviction Stock
  const topConvictionStock =
    watchlistStocks.length > 0
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl pl-9 pr-3 py-1.5 text-xs font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500"
            />
          </div>

          <div className="flex items-center gap-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-2.5 py-1.5 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300">
            ...
            >
              <option value="return" className="bg-white dark:bg-slate-900">Sort by 7D Forecast</option>
              <option value="price" className="bg-white dark:bg-slate-900">Sort by Price</option>
              <option value="symbol" className="bg-white dark:bg-slate-900">Sort by Ticker</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 p-5 shadow-2xs">
          ...
          <span className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">
            {watchlistStocks.length > 0 ? avg7DReturnFormatted : 'N/A'}
          </span>
        </div>
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 p-5 shadow-2xs">
      ...
      {filteredStocks.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 p-12 text-center shadow-xs">
          ...
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-1">
            {watchlistStocks.length === 0 ? 'Your Watchlist is empty' : 'No matching saved stocks found'}
          </h3>
                ...
                <div>
                  ...
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <button
                      ...
                      className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 transition-colors rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/50 cursor-pointer"
